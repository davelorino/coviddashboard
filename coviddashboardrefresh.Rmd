...
cov_7dayfiltered_textonly <- as.character(cov_7dayfiltered$Hit.Sentence)
...
kw_branded_top25 <-
  ...
  mutate("Traffic Share" = as.double(str_remove(`Organic`, "%"))) %>% 
  top_n(25, `TrafficShare`) %>%
  mutate(Destination = name, `Search Terms` = `Search terms`) %>%
  select(`Search Terms`, `Traffic Share`, name) 

library(reactable)
library(htmltools)
bar_chart <- function(label, destination, width = "100%", height = "16px", fill = "#00bfc4", background = NULL) {
...
destinations_test <- reactable(
  ...
  "Traffic Share" = colDef(
    cell = function(value) {
      width <- paste0(value / max(kw_branded_top25$`Traffic Share`) * 100, "%")
