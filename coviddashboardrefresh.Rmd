...
kw_unbranded <- read_csv("~/NetBaseApi/coviddashboard//search_keywords_unbranded.csv")

url_cols <- kw_branded %>%
  select(contains(".com")) %>%
  names()

url_cols_unbranded <- kw_unbranded %>%
  select(contains(".com")) %>%
  names()

kw_branded_top25 <- kw_branded %>%
...
kw_unbranded_top25 <- kw_unbranded %>%
  pivot_longer(cols = url_cols_unbranded) %>%
  filter(!value == "NA") %>%
  filter(!`Total Traffic Share` == "NA") %>%
  mutate(TrafficShare = as.double(str_remove(`Total Traffic Share`, "%"))) %>% 
  mutate("_TrafficShare" = as.double(str_remove(`value`, "%"))) %>%
...
saveRDS(kw_branded_top25, "~/NetBaseApi/coviddashboard/kw_branded_top25.rds")
saveRDS(kw_unbranded_top25, "~/NetBaseApi/coviddashboard/kw_unbranded_top25.rds")

test <- readRDS("~/NetBaseApi/coviddashboard/kw_unbranded_top25.rds")
...
renderKeywordsBankingSectorTableUnbranded(kw_unbranded_top25)
...
```{r}
branded_overall_percentage <- kw_branded_top25 %>%
  ungroup() %>%
  group_by(Destination) %>%
  summarise(SumTrafficShare = sum(`_TrafficShare`), .groups = "keep") %>%
  ungroup() %>%
  mutate(`Overall Traffic Share` = round(`SumTrafficShare` / sum(`SumTrafficShare`) * 100, 2)) %>%
  ungroup() %>%
  mutate(Category = "Destination") %>%
  ungroup() %>%
  ggplot(aes(x = Category, y = `Overall Traffic Share`, fill = Destination)) +
    geom_col(position = "stack") +
  ggtitle("Banking Sector Share of Voice") +
  coord_flip(clip = "off") +
  scale_fill_manual(values = c("#004164", "#f2c40e", "#3fc1c9", "#78be20", "#f6182a")) +
  theme(plot.title = element_text(size = 10, vjust = -2)) +
  theme(axis.text.y=element_blank()) +
  theme(title = element_text(color = "#FFFFFF"))

legend_features_banking_keywords <- list(
        font = list(
            size = 8,
            color = "#FFFFFF"),
        bgcolor = "#212121",
...
branded_overall_sov <- ggplotly(branded_overall_percentage) %>%
  config(displayModeBar = FALSE) %>%
     plotly::layout(
                     #title = "Percentage of Traffic Share",
                     yaxis = list(title = "", color = "#FFFFFF")
                     ,xaxis = list(title = "", color = "#FFFFFF"),
                      legend = legend_features_banking_keywords,          
                     paper_bgcolor='#1a1a1a',
                     plot_bgcolor='#1a1a1a'
                 )


saveRDS(branded_overall_sov, "~/NetBaseApi/coviddashboard/branded_overall_sov.rds")
...
unbranded_overall_percentage <- kw_unbranded_top25 %>%
  ungroup() %>%
  group_by(Destination) %>%
  summarise(SumTrafficShare = sum(`_TrafficShare`), .groups = "keep") %>%
  ungroup() %>%
  ...
  ggtitle("Banking Sector Share of Voice") +
  coord_flip(clip = "off") +
  scale_fill_manual(values = c("#004164", "#f2c40e", "#3fc1c9", "#f6182a")) +
  theme(plot.title = element_text(size = 10, vjust = -2)) +
  theme(axis.text.y=element_blank()) +
  theme(title = element_text(color = "#FFFFFF"))
...
unbranded_overall_sov <- ggplotly(unbranded_overall_percentage) %>%
  config(displayModeBar = FALSE) %>%
     plotly::layout(
                     #title = "Banking Sector Share of Voice",
                     yaxis = list(title = "", color = "#FFFFFF")
                     ,xaxis = list(title = "", color = "#FFFFFF"),
                      legend = legend_features_banking_keywords,          
                     paper_bgcolor='#1a1a1a',
...
unbranded_overall_sov

saveRDS(unbranded_overall_sov, "~/NetBaseApi/coviddashboard/unbranded_overall_sov.rds")
